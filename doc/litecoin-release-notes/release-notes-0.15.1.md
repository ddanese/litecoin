zDatacoin Core version *0.15.1* is now available from:

  <https://download.zdatacoin.org/zdatacoin-0.15.1/>
...
A number of changes to the way zDatacoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 gigs per kB (0.00001 ZDATA). This is to prepare for dropping the minimum transaction fee to 0.1 gigs per kB (0.0001 ZDATA) in the next release.
