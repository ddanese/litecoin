...
const char * const BITCOIN_CONF_FILENAME = "zdatacoin.conf";
...
#else
    const char* pszModule = "zdatacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "zDatacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/zDatacoin";
#else
    // Unix
    return pathRet / ".zdatacoin";
#endif
#endif
}
